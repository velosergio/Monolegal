...
- [X] T028 Ejecutar la validación de `specs/016-transition-form-donut-dashboard/quickstart.md` (escenarios A–D). **Automatizado** (2026-06-30): escenarios A (dashboard como inicio, navegación activa, redirección de `/dashboard` y rutas desconocidas a `/`) y B (donut: total al centro + leyenda por estado) en `frontend/e2e/dashboard-home.spec.ts` 4/4 verde; escenario C cubierto por `manual-transition.spec.ts`. **Pendiente humano**: validación visual del escenario D (claro/oscuro, responsive móvil/escritorio, lector de pantalla).
