...
import { createInvoiceItemForm, type InvoiceDetail, type InvoiceFormValues } from '../types'
...
function emptyValues(): InvoiceFormValues {
  return { clientId: '', dueDate: '', items: [createInvoiceItemForm()] }
}
...
/** Valores iniciales del formulario derivados de la factura a editar (o vacíos en alta). */
function initialValues(invoice?: InvoiceDetail | null): InvoiceFormValues {
  return invoice
    ? {
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-w-2xl">
        {/* La key remonta el formulario al cambiar de factura, reiniciando su estado sin efectos. */}
        {open ? (
          <InvoiceForm key={invoice?.id ?? 'new'} invoice={invoice} onClose={onClose} />
        ) : null}
      </DialogContent>
    </Dialog>
  )
...
function InvoiceForm({ invoice, onClose }: InvoiceFormProps) {
  ...
  const [values, setValues] = useState<InvoiceFormValues>(() => initialValues(invoice))
